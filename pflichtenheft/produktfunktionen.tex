...
\vspace{.5cm}
\todo{grid can be displayed}
\begin{description}
	\item[\textlabel{/FR000/}{FR:F000}] \textbf{\textlabel{Grid}{FR:FT000}} \\
	Vertices are positioned using a grid system that the framework provides. The size of the grid can be freely set by the developer depending on the game that is being implemented. The grid system can either be visible in the \nameref{framework-gui} or invisible, again, depending on the needs of the developer.
	\item[\textlabel{/FR010/}{FR:F010}] \textbf{\textlabel{Event listeners}{FR:FT010}} \\
	The framework already implements event listeners for user interactions. Mouse clicks as well as keyboard strokes can be used to interact with the game, but can also be disabled, if wanted to.
  	\item[\textlabel{/FR020/}{FR:F020}] \textbf{\textlabel{Add/Remove/Move vertices/edges}{FR:FT020}} \todo{Select edges is optional; move is optional}  \\
	...
	\item[\textlabel{/FR060/}{FR:F060}] \textbf{\textlabel{Check for planarity in displayed graph}{FR:FT060}} \\
	For a given graph, it is possible to verify, if an edge between two given unconnected vertices exists, so that the resulting graph is planar. \todo{we display planar graphs}
