...
\begin{description}
	...
	The {\graphioli}  already implements event listeners for user interactions. Mouse clicks as well as keyboard strokes can be used to interact with the game, but can also be disabled, if wanted to.
  	\item[\textlabel{/FR010/}{FR:010}] \textbf{\textlabel{Add/Remove/Move vertices/edges}{FR:T010}} \todo{Select edges is optional; move is optional}  \\
	...
	For a given graph, it is possible to verify, if an edge between two given unconnected vertices exists, so that the resulting graph is planar. \todo{we display planar graphs}
 	...
 	The \nameref{FR:game-explorer} \todo{change game-explorer to gui}displays all graph games, which where implemented using the {\graphioli}.
