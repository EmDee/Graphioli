...
public class GraphCanvas extends JPanel {
	...
	protected void paintComponent(Graphics g) {
		...
		for(Vertex v: graph.getVertices()) {
			...
			g2d.drawImage(vertex.getBufferedImage(),
					(vertex.getGridPoint().getPositionX() * horizontalGridScale) - (visualGrid.getVisualVertexSize() / 2),
					(vertex.getGridPoint().getPositionY() * verticalGridScale) - (visualGrid.getVisualVertexSize() / 2),
					visualGrid.getVisualVertexSize(),
					visualGrid.getVisualVertexSize(),
					null);
		...
		for(Edge edge: graph.getEdges()) {
			/*
			 * Size of the VisualEdge bzgl. grid
			 */
			int xsize = 0;
			int ysize = 0;
